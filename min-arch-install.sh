...
_custom_reverse_search_dmenu() {
    local result=$(HISTTIMEFORMAT= history | sed 's/^ *[0-9]* *//' | grep -F -- "$READLINE_LINE" | tac | awk '!a[$0]++' | dmenu -l 10 -p "History> ")
    [[ -n $result ]] && READLINE_LINE="$result" && READLINE_POINT=${#READLINE_LINE}
}
